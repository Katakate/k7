"""k7d VM ops through the centralized HTTP API.
...
op to the k7-agent DaemonSet pod on that node — so
...
from .test_api import _api_pod_ready, _generate_test_api_key, _get_api_base_url, _tls_verify, httpx
...
def _ready_node_names() -> list[str]:
    return [n["metadata"]["name"] for n in _kubectl_json("get", "nodes")["items"] if _node_ready(n)]
...
def api_base_url() -> str:
    ...
    for _ in range(30):
        try:
            if httpx.get(f"{url}/health", timeout=3, verify=_tls_verify()).status_code == 200:
                return url
...
class TestK7dViaApi:
    def test_k7d_pause_resume_fork_via_api_colocated(
        ...
        ``node`` field so clients can see placement."""
        name = "api-k7d-vmops"
    ...
    def test_k7d_vm_ops_on_remote_node_via_agent(
        ...
        """a k7d sandbox on a DIFFERENT node than the API pod is
        ...
        source's node (cross-node fork is future k7d work, not built)."""
        nodes = _kubectl_json("get", "nodes")
        other = [n["metadata"]["name"] for n in nodes["items"] if n["metadata"]["name"] != api_node and _node_ready(n)]
        if not other:
            pytest.skip("no other Ready node — remote k7-agent path not testable")
...
class TestK7Agent:
    def test_agent_rejects_unauthenticated_requests(self, api_node: str):
        """the agent must reject requests without the shared token
    ...
    def test_nodes_storage_sdk(self, api_base_url: str, api_key: str):
        """SDK ``Client.nodes_storage()`` hits the same endpoint."""
        from k7_sdk import Client

        nodes = _ready_node_names()
        data = Client(endpoint=api_base_url, api_key=api_key, verify_ssl=_tls_verify()).nodes_storage()
        for node in nodes:
